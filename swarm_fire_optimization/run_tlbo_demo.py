"""
run_tlbo_demo.py
TLBO on Team 39 swarm-fire problem, optimizing U and Δt together.

Run from project root:
  python -m swarm_fire_optimization.run_tlbo_demo
"""
...
from swarm_fire_optimization.tlbo import teaching_learning_based_optimization


# -------------------- Problem Definitions (Case Studies) -------------------- #

def make_problem_small():
    """Case A: small horizon, few robots."""
...
# ------------------------ TLBO Runner for One Case ------------------------ #

def run_case(label, W, robot, sim, X0,
             dt_bounds=(0.20, 0.80),
             seed=0):
    """
    Run TLBO on a given case study, save results, and produce a trajectory plot.
    """
    # Initial guess (same style as GA / PSO / SA)
    U0 = generate_random_controls(sim, robot, seed=42)
    dt0 = float(0.5 * (dt_bounds[0] + dt_bounds[1]))
    ...
    # Cost wrapper for TLBO (sets sim.dt_var each evaluation)
    def cost(U, dt_var):
    ...
    # ---- TLBO Parameters ---- #
    U_best, dt_best, log = teaching_learning_based_optimization(
        cost_fn=cost,
        U0=U0,
        dt0=dt0,
        vmax=robot.vmax,
    ...
    out = compute_fitness(
        X0, U_best, W, robot, sim,
        alpha=alpha,
        penalty_weights=penalty_weights
    )
    X = out["X"]

    print(f"\n[{label}] TLBO finished: best J={out['J_total']:.6f}, "
          f"dt*={dt_best:.3f}s, iters={log['iters']}")
    keys = ["J_cov", "J_dist", "J_bal", "J_time",
            "P_speed", "P_dist", "P_conn", "P_energy", "P_ws"]
    for k in keys:
        print(f"  {k:>10s}: {out[k]:.6f}")

    # Plot trajectories vs fire
    fig, ax = plot_paths_with_fire(
        X, sim, W,
        title=f"TLBO Result – {label} (dt={dt_best:.2f}s)",
        steps=(0, sim.T // 2, sim.T),
    )
    png = f"tlbo_{label}.png"
