"""
run_ga_demo.py
GA on Team 39 swarm-fire problem, optimizing U and Δt together.

Run from project root:
  python -m swarm_fire_optimization.run_ga_demo
"""
...
from swarm_fire_optimization.ga import genetic_algorithm
...
def make_problem_full():
    """Case B: original full problem (same as SA)."""
    W = Workspace(0.0, 12.0, 0.0, 10.0, obstacles=[(7.5, 3.0, 8.5, 6.0)])
    robot = RobotParams(vmax=1.6, rs=2.0, rc=3.0, Emax=25.0, sigma=1.0, dmin=0.8)
...
# ------------------------ GA Runner for One Case ------------------------ #

def run_case(label, W, robot, sim, X0,
             dt_bounds=(0.20, 0.80),
             seed=0):
    """
    Run GA on a given case study, save results, and produce a trajectory plot.
    """
    # Initial guess (same style as SA)
    ...
    out = compute_fitness(X0, U_best, W, robot, sim,
                          alpha=alpha, penalty_weights=penalty_weights)
    X = out["X"]

    print(f"\n[{label}] GA finished: best J={out['J_total']:.6f}, dt*={dt_best:.3f}s, "
          f"gens={log['iters']}")
    ...
    fig, ax = plot_paths_with_fire(
        X, sim, W,
        title=f"GA Result – {label} (dt={dt_best:.2f}s)",
