"""
run_pso_demo.py
PSO on Team 39 swarm-fire problem, optimizing U and Δt together.

Run from project root:
  python -m swarm_fire_optimization.run_pso_demo
"""
...
from swarm_fire_optimization.pso import particle_swarm_optimization
...
def make_problem_full():
    """Case B: original full problem (same as SA/GA)."""
    W = Workspace(0.0, 12.0, 0.0, 10.0, obstacles=[(7.5, 3.0, 8.5, 6.0)])
    robot = RobotParams(vmax=1.6, rs=2.0, rc=3.0, Emax=25.0, sigma=1.0, dmin=0.8)
...
# ------------------------ PSO Runner for One Case ------------------------ #

def run_case(label, W, robot, sim, X0,
             dt_bounds=(0.20, 0.80),
             seed=0):
    """
    Run PSO on a given case study, save results, and produce a trajectory plot.
    """
    # Initial guess (same style as SA / GA)
    ...
    # Cost wrapper for PSO (sets sim.dt_var each evaluation)
    def cost(U, dt_var):
    ...
    # ---- PSO Parameters ---- #
    U_best, dt_best, log = particle_swarm_optimization(
        cost_fn=cost,
        U0=U0,
        dt0=dt0,
        vmax=robot.vmax,
    ...
    print(f"\n[{label}] PSO finished: best J={out['J_total']:.6f}, "
    ...
    fig, ax = plot_paths_with_fire(
        X, sim, W,
        title=f"PSO Result – {label} (dt={dt_best:.2f}s)",
        steps=(0, sim.T // 2, sim.T),
    )
    png = f"pso_{label}.png"
