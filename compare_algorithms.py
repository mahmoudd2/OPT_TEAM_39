...
def plot_objective_convergence(sa, ga, out_folder):
    # evaluation-level objective-only histories
    sa_obj = sa["eval_obj_hist"]
    ga_obj = ga["eval_obj_hist"]

    plt.figure(figsize=(8,5))
    plt.plot(np.arange(len(sa_obj)), sa_obj, label="SA", linewidth=1.8)
    plt.plot(np.arange(len(ga_obj)), ga_obj, label="GA", linewidth=1.8)
    plt.xlabel("Evaluation index")
    plt.ylabel("Objective value J_obj")
...
def compare_case(case_name, T, N):
    ...
    # 1b. Objective-only convergence (smoother curve)
    plot_objective_convergence(sa, ga, out_folder)
