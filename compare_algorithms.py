...
def load_results(case):
    ...
    pso = np.load(f"pso_{case}.npz", allow_pickle=True)     # NEW
    return sa, ga, pso
...
def plot_convergence(sa_log, ga_log, pso_log, out_folder):
    ...
    plt.title("Convergence Curve – SA vs GA vs PSO")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def plot_objective_convergence(sa, ga, pso, out_folder):
...
def plot_objectives(out_sa, out_ga, out_pso, out_folder):
    ...
    plt.bar(x - width, sa_vals, width=width, label="SA")
    plt.bar(x, ga_vals, width=width, label="GA")
    plt.bar(x + width, pso_vals, width=width, label="PSO")
...
def plot_penalties(out_sa, out_ga, out_pso, out_folder):
...
def plot_trajectories(sa, ga, pso, case, W, sim, out_folder):
...
def compare_case(case_name, T, N):
    ...
    sa, ga, pso = load_results(case_name)
    ...
    plot_convergence(sa["log"].item(), ga["log"].item(), pso["log"].item(), out_folder)
    plot_objective_convergence(sa, ga, pso, out_folder)
    plot_objectives(out_sa, out_ga, out_pso, out_folder)
    plot_penalties(out_sa, out_ga, out_pso, out_folder)
    plot_trajectories(sa, ga, pso, case_name, W, sim, out_folder)
